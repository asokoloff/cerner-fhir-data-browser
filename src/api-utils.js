...
const API_HOST = 'https://fhir-open.sandboxcerner.com/dstu2/0b8a0111-e8e6-4c26-a91c-5069cbc6b1ca/'
...
export function getPatient(patientId) {
    return axios.get(`${API_HOST}${PATIENT_ENDPOINT}${patientId}`)
    .catch(e => {
        throw(e.response);
    })
}
...
export function getConditions(patientId) {
    return axios.get(`${API_HOST}${CONDITION_ENDPOINT}?patient=${patientId}`)     
    .catch(e => {
        throw(e.response);
    })
...
export function extractPatientConditions(response) {
    ...
    .map(c => {
        ...
        const encodedSearchString = encodeURIComponent(path(['resource', 'code', 'text'], c));
